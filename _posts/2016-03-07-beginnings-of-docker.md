...
ここではお試しとして、docker-machine.exeだけダウンロードします。

1.  まず、[ココ](https://github.com/docker/machine/releases/)から`docker-machine-Windows-x86_64.exe`をダウンロードします。

2.  ダウンロードしたら下記コマンドを実行して、正常に動作するか確認します。お試しなので、実行ファイルのリネームとか`PATH`設定はスキップします。
    ~~~
...
3.  下記コマンドを実行して、DockerホストをダウンロードおよびVirtualBoxに追加します。
    ~~~
